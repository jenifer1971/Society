...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    """
    Create an LLM model via the provider configured in LLM_PROVIDER.

    use_boost is kept for API compatibility but has no effect — enterprises
    achieve parallelism through their provider's own rate limits / quotas.
    """
    from llm_provider_helper import create_camel_model
    return create_camel_model()
